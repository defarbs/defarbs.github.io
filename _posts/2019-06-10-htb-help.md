...
Returning to the box, I wanted to make sure I would be able to transfer the file to myself. Unfortunately, `curl` is not installed, but `wget` is! I hosted the `exploit.c` file from the CVE with a `SimpleHTTPServer` via `python` and then grab the file on the Help machine with `wget`. It should look something like this:
